...
bool DBOXUIManager::Create(DBOXUIPROPERTIES properties)
{
	...
	HRESULT hr = D3DXCreateTextureFromFileEx(props.pDevice , props.TextureName.c_str() , D3DX_DEFAULT , D3DX_DEFAULT , D3DX_DEFAULT , 0 , 
...
void DBOXUIManager::UpdateContent()
{
	...
	int AlphaH = (int) ( transfBntRects[0].bottom - transfBntRects[0].top ) - 14;
	int AlphaW = (int) ( (float)AlphaH* 0.4f) ;

	ButtonFont = DBOXCreateFont(props.FontFace , props.pDevice , AlphaW , AlphaH , 700 , props.FontColor);
	...
	CheckboxFont = DBOXCreateFont(props.FontFace , props.pDevice , AlphaW , AlphaH , 700 , props.FontColor);

	//Textbox Aplpha 

	AlphaH = (int) ( transfTextboxRect.bottom - transfTextboxRect.top ) - 14;
	AlphaW = (int) ( (float)AlphaH*0.4f) ;

	TextboxFont = DBOXCreateFont(props.FontFace , props.pDevice , AlphaW , AlphaH , 700 , props.FontColor);
}
...
void DBOXUIButton::Create(DBOXUIManager* mngr , tstring Text , lPoint Position)
{
	this->pManager = mngr;
...
void DBOXUIButton::Update()
{
	...
	pManager->ButtonFont->DrawTextEx(this->InfoState.strValue.c_str(), DT_VCENTER|DT_CENTER , (RECT*)(&boundingRect) );
...
void DBOXUITextbox::Create(DBOXUIManager* mngr , int limit  , lPoint Position , tstring Text)
{
...
void DBOXUITextbox::Update()
{
	...
	tstring textToDraw = this->InfoState.strValue;
	...
	if(this->InfoState.Active && !InfoState.Locked)
		pManager->TextboxFont->DrawTextEx(textToDraw.c_str() , DT_RIGHT | DT_VCENTER , (RECT*)&boundingRect);
	else
		pManager->TextboxFont->DrawTextEx(textToDraw.c_str() , DT_CENTER | DT_VCENTER , (RECT*)&boundingRect);
...
DBOXUIObject* DBOXUICreateTextbox(DBOXUIManager* mngr , int limit , lPoint Position , tstring Text)
{
...
void DBOXUICheckbox::Create(DBOXUIManager* mngr, lPoint Position, tstring Text)
{
	this->pManager = mngr;
...
void DBOXUICheckbox::Update()
{
	...
	this->pManager->CheckboxFont->DrawTextEx(this->InfoState.strValue.c_str() , DT_VCENTER | DT_LEFT , (RECT*)&boundingRect);
