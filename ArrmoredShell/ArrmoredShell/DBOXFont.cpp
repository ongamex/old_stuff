...
DBOXFontManager lDBOXFontManger;
...
HRESULT DBOXFont::Create(tstring FontFace ,LPDIRECT3DDEVICE9 d3ddev, int Width , int Height , int Weight , DWORD TextColor)
{
	...
	{
		lDBOXFontManger.PushObject((void*)this);
		return  DBOXFONT_OK;
...
void DBOXFont::Release()
{
	...
	Font = NULL;
	lDBOXFontManger.PopObject((void*)this);
	return;
}

HRESULT DBOXFont::DrawText(tstring Text , int x , int y)
{
 	UINT StrLenght = tstrlen(Text.c_str());
...
//dbox Font resource manager

void DBOXFontManager::OnLostDevice(){

	int Size = mBuffer.size();
	for(int i = 0; i < Size; ++i){
		if(mBuffer[i] != NULL)mBuffer[i]->GetFont()->OnLostDevice();
	}
	return;
}
void DBOXFontManager::OnResetDevice(){
	int Size = mBuffer.size();
	for(int i = 0; i < Size; ++i){
		if(mBuffer[i] != NULL)mBuffer[i]->GetFont()->OnResetDevice();
	}
	return;
}

void DBOXFontManager::PushObject(void* pArg){ if( pArg ) mBuffer.push_back((LPDBOXFONT)pArg ) ;} 

void DBOXFontManager::PopObject(void* pArg){

	LPDBOXFONT pFont = (LPDBOXFONT)pArg;
