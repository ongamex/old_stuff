...
void DBOXApplication::Create(const TCHAR* WindowName , bool Windowed , long Width , long Height)
{
	...
	DWORD Style = (Windowed) ? WS_OVERLAPPED | WS_SYSMENU | WS_VISIBLE /*| WS_CLIPCHILDREN */: WS_POPUP | WS_SYSMENU | WS_VISIBLE /*| WS_CLIPCHILDREN*/;

	RECT ClientRect = { 0 , 0 , Width , Height};
	AdjustWindowRect(&ClientRect , Style , false);

	m_hWnd = CreateWindowEx( 0 , WinCl.lpszClassName , WindowName , Style , CW_USEDEFAULT , CW_USEDEFAULT , ClientRect.right , ClientRect.bottom , 
...
void DBOXApplication::ResizeClient(long Width , long Height)
{
	...
	MoveWindow(m_hWnd,rcWindow.left, rcWindow.top, Width + ptDiff.x, Height + ptDiff.y, TRUE);
